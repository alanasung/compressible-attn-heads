...
|---|---|---|
| Config schema | `progattn.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `progattn.configs.loader` | Hydra compose API |
| Device | `progattn.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `progattn.models.registry` | Pinned revisions + hardware fit |
| Hooks | `progattn.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `progattn.models.generation` | Chat template path recording |
| Data | `progattn.data` | Synthetic items, splits, manifests |
| Evaluation | `progattn.evaluation` | Metrics, failure harness, protocols |
| Ablation | `progattn.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `progattn.reporting` | Aggregate → MD/TeX/figures |
| Cache | `progattn.cache` | Versioned artifact store |
...
SPAR posting: https://sparai.org/projects/f26/reci1DhApjFAtQx7L
...
`src/progattn/progattn/` are preserved across regeneration.
...
E01 sweeps a GPT-2-shaped 12×12 head grid against `previous_token`,
`positional_offset`, and `bos_attend` before later stages are motivated. Valid
layer indices are **0..11**.

### Surgery vs masking

Genuine removal slices Q/K columns from fused `c_attn` and rebuilds a narrower
projection. Parameter counts are measured on the rebuilt module with a numerical
equivalence check. Masking is never reported as parameter reduction. Wall-clock
is an eager-mode lower bound, not a production kernel claim.
